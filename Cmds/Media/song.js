module.exports = async (messageDetails) => {
  ...
  try {
    ...
    if (searchResults && searchResults.videos.length > 0) {
      ...
      // Fetch download link from an external API
      const response = await fetch(`https://dark-yasiya-api-new.vercel.app/download/ytmp3?url=${encodeURIComponent(videoUrl)}&apikey=cracker`);
      ...
      if (downloadData.status === 200 && downloadData.success) {
        ...
        await botClient.sendMessage(chat, videoInfoMessage, {
          quoted: message
        });
