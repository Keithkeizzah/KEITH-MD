...
function createProxyConfig(proxy) {
  if (!proxy || proxy.length == 0) return void 0;
...
var ChatCompletionHandler = class {
  ...
  async runPreprocessing(messages, options, provider) {
    let text = "";
    if (options) options.stream = false;
...
var TranslationHandler = class {
  ...
  async generateTranslation(options) {
    ...
    if (!provider) provider = getProviderFromList(this.providersList, provider_log2, { model: void 0, debug });
    else if (debug) runLog(provider_log2.success, `Provider found: ${provider.name}`, true);
    ...
    if (debug) runLog(provider_log2.await, `Fetching data from the provider: ${provider.name}`);
    const data = await provider.fetchData(options);
    if (debug) runLog(provider_log2.success, `Data was successfully fetched from the "${provider.name}" provider`, true);
    return data;
  }
};
...
var ImageGenerationHandler = class {
  ...
  async imageGeneration(prompt, options) {
    let { debug, provider } = options || {};
    if (!provider) provider = getProviderFromList(this.providersList, provider_log3, { model: void 0, debug });
    else if (debug) runLog(provider_log3.success, `Provider found: ${provider.name}`, true);
    ...
    if (debug) runLog(provider_log3.await, `Fetching data from the provider: ${provider.name}`);
    const data = await provider.fetchData(prompt, options?.providerOptions);
    if (debug) runLog(provider_log3.success, `Data was successfully fetched from the "${provider.name}" provider`, true);
    return data;
  }
...
var GiftedGpt = class {
  ...
  async chatCompletion(messages, options) {
    if (options) OptionsHandler_default.checkChatCompletionOptions(options);
  ...
  async translation(options) {
    if (options) OptionsHandler_default.checkTranslationOptions(options);
   ...
   */
  async imageGeneration(prompt, options) {
    if (options) OptionsHandler_default.checkImageGenerationOptions(options);
    return await this.imageGenerationHandler.imageGeneration(prompt, options);
  }
};
var GiftedGpt_default = GiftedGpt;
export {
  GiftedGpt_default as GiftedGpt,
  chunkProcessor
};
