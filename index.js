...
const {
  ...
  mycode, author, antibad, packname, url, gurl, herokuapi, herokuapikey, anticall, dev, antilink, gcpresence, antionce, antitag, antidelete, autolike,
} = require("./settings");
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      if (command) {
        ...
        if (commandObj) {
          await commandObj.execute({ client, m, mode, text, totalCommands, botname, url, gurl, prefix, groupAdmin, getGroupAdmins, args, groupName, groupMetadata, herokuapi, herokuapikey, participants, isOwner, pushname, botNumber, itsMe, store, isAdmin, isBotAdmin });
        }
      }
    } catch (err) {
