  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      if (command) {
        ...
        if (commandObj) {
          await commandObj.execute({ client, m, mode, text, totalCommands, botname, url, gurl, prefix, groupAdmin, getGroupAdmins, args, groupName, groupMetadata, herokuAppname, herokuapikey, participants, isOwner, pushname, botNumber, itsMe, store, isAdmin, isBotAdmin });
        }
      }
    } catch (err) {
